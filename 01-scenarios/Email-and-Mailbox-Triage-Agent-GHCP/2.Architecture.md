...
flowchart LR
    ...
    MBX --> PA --> NORM --> ATT
    ATT -. supported invocation required .-> CLS
    CLS --> REF --> REVIEW --> GATE
...
The diagram describes the delivery architecture. Mail-arrival invocation of the GHCP agent
is a compatibility gate, not a connection supplied by the reference files. An agent calling
a workflow does not prove that a mail-triggered workflow can call that agent. Confirm the
supported invocation and identity in [Phase 1](3.Runbook.md#phase-1---pre-processing-week-2).
