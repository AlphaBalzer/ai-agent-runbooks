...
consult the agreed accounting reference where needed and explain missing or conflicting facts.
...
| **Resolve a question** | Use the agreed accounting reference when needed; do not invent a code. |
...
The [runbook](3.Runbook.md#1-create-the-ghcp-agent) contains the proposed `invoice-review` skill
and concise agent instructions. Its original-flow bindings still need implementation and
validation. The older two-skill text pilot is preserved as technical history, not the default
build path.
