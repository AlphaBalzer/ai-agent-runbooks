...
this agent supports the process from an emailed supplier invoice to a payment request form and
human approval. SharePoint holds the documents; Dataverse holds the transaction and its status.

The GitHub Copilot harness adds evidence-based investigation before form preparation. The agent
identifies missing or conflicting invoice details, consults approved references and explains what
supports the request. Workflows enforce financial checks and writes; an authorized person makes
...
|---|---|
| **Extract invoice details** | Associate material values with source quotes and flag missing or conflicting information. |
| **Check supporting information** | Consult approved vendor, purchase-order, receipt, GL and cost-centre references, then reassess the request. |
| **Prepare the payment request** | Explain the evidence and validation findings alongside the form or exception. |
| **Reuse the approach** | Use focused extraction and review skills across the agreed invoice types. |

The two skills are [invoice-evidence-extraction](Skills/invoice-evidence-extraction/SKILL.md) and
[payment-request-review](Skills/payment-request-review/SKILL.md). Skills describe how the agent
works; tools and their connections provide the actual operations and access.
...
An invoice arrives in the designated mailbox. The intake workflow saves the attachment to
SharePoint and creates a Dataverse transaction with its source references. The agent reads that
invoice, extracts its details and checks the relevant approved finance information.
...
```mermaid
flowchart LR
    A["Invoice email"] --> B["Save source<br/>SharePoint + Dataverse"]
    B --> C["Extract and investigate<br/>invoice evidence + approved references"]
...
This is the complete scenario design. The [delivery status](0.Resources/README.md#delivery-status)
distinguishes it from the components and outcomes demonstrated so far.
...
- Invoice email intake, supported attachment storage and linked Dataverse records.
- Source-linked extraction of invoice fields, with missing or conflicting facts made visible.
- Approved GL, cost-centre and applicable vendor, purchase-order and receipt checks.
- Pre-filled payment request forms, including an approved HTML template where required.
